...
@section('content')
    ...
    <div class="bg-cyan-50 relative min-h-screen">
        <img src="{{ asset('images/payment-deco.png') }}" alt="payment-deco"
            class="hidden md:block absolute top-0 left-0 z-0 w-full pointer-events-none">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8 z-10 relative">
            {{-- Back Button --}}
            ...
            <form action="{{ route('proses-pembayaran') }}" method="POST">
                ...
                <div class="flex flex-col lg:flex-row justify-center gap-16">
                    ...
                    <div class="bg-white p-6 rounded-lg w-full lg:w-2/5 xl:w-1/3 ">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="bg-gray-50 border border-gray-300 rounded-md p-3">
                                    <span
                                        class="font-quicksand text-gray-700">{{ Auth::user()->name ?? 'Nama Siswa' }}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="bg-gray-50 border border-gray-300 rounded-md p-3">
                                    <span
                                        class="font-quicksand text-gray-700">{{ Auth::user()->phone_number ?? 'Nomor HP tidak tersedia' }}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="bg-gray-50 border border-gray-300 rounded-md p-3">
                                    <span
                                        class="font-quicksand text-gray-700">{{ Auth::user()->parents_phone_number ?? 'Nomor HP Orang Tua tidak tersedia' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <input type="hidden" name="parent_name"
                            value="@php echo Auth::user()->name ? 'Mr/Mrs ' . explode(' ', Auth::user()->name)[0] : ''; @endphp">
                    ...
                    <div
                        class="bg-white p-6 rounded-lg w-full lg:w-2/5 xl:w-1/3 self-start">
                        ...
                        <ul class="space-y-3 mb-6">
                            @foreach ($benefits as $benefit)
                                <li class="flex items-start gap-2">
                                    <svg class="w-5 h-5 text-[#1ACC46] shrink-0" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div class="mb-4 p-3 bg-gray-50 rounded-lg">
                            <div class="flex flex-wrap gap-2">
                                @if ($packet->type === 'premium')
                                    <span
                                        class="px-2 py-1 bg-yellow-400 text-white text-xs rounded-full font-quicksand font-bold">Premium</span>
                                @endif
                                <span
                                    class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full font-quicksand font-bold">
                                    Kelas {{ $packet->grade }}
                                </span>
                                <span
                                    class="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full font-quicksand font-bold">
                                    {{ $packet->subject }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 pt-4">
                            ...
                            <div id="discount-row"
                                class="flex justify-between font-quicksand text-base {{ $discountPercentage > 0 ? '' : 'hidden' }}">
                                <span class="text-red-500">Diskon (<span
                                        id="discount-percentage">{{ $discountPercentage }}</span>%)</span>
                                <span class="text-red-500"
                                    id="discount-amount">-Rp{{ number_format($discountAmount, 0, ',', '.') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="border-t mt-4 pt-4">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <span class="font-quicksand font-bold text-xl"
                                    id="final-price">Rp{{ number_format($finalPrice, 0, ',', '.') }}</span>
                            </div>
                            <button type="submit"
                        ...
                        <input type="hidden" name="discount_percentage" id="discount-percentage-input"

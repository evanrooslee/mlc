...
@section('content')
    <div class="relative mb-10">
        ...
        <div class="mx-4 md:mx-8 my-4">
            ...
            <div class="flex flex-col inset-shadow-[0px_2px_4px_rgba(0,0,0,0.25)] rounded-lg border border-[#C9C9C9] mb-6">
                ...
                @if ($allOtherPackets->isNotEmpty())
                    <div class="flex md:hidden overflow-x-auto pb-4 gap-4 m-3 snap-x snap-mandatory">
                        @foreach ($allOtherPackets as $packet)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden max-w-[200px] snap-center flex-shrink-0">
                                <img src="{{ $packet->image_url }}" alt="{{ $packet->title }}"
                                    class="w-full h-40 object-cover">
                                <div class="p-2">
                ...
                @if ($otherPackets->isNotEmpty())
                    <div class="hidden md:grid md:grid-cols-3 lg:grid-cols-5 gap-4 m-3">
                        @foreach ($otherPackets as $packet)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <img src="{{ $packet->image_url }}" alt="{{ $packet->title }}"
                                    class="w-full h-40 object-cover">
                                <div class="p-2">
                                    <h3 class="font-medium text-left">{{ $packet->title }}</h3>
                                </div>
                            </div>
                        @endforeach
                    </div>
                @elseif($allOtherPackets->isEmpty())
                    <p class="text-gray-500 p-3">Tidak ada paket lainnya yang tersedia saat ini.</p>
                @endif
        </div>
        <div class="hidden md:flex justify-center items-center mt-5 gap-8 text-3xl">
            ...
            <a href="{{ $otherPackets->nextPageUrl() }}" class="rotate-180">
                <svg xmlns="http://www.w3.org/2000/svg" width="11" height="18" viewBox="0 0 11 18" fill="none"
                    class="rotate-180">
                    <path fill-rule="evenodd" clip-rule="evenodd"

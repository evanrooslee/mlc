        ...
        <div class="mx-8 my-4">
            ...
            <div class="flex flex-col inset-shadow-[0px_2px_4px_rgba(0,0,0,0.25)] rounded-lg border border-[#C9C9C9] mb-6">
                ...
                @if ($activePackets->isNotEmpty())
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-2.5">
                        ...
                        @foreach ($otherPackets as $packet)
                            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                                <img src="{{ $packet->image_url }}" alt="{{ $packet->title }}"
                                    class="w-full h-40 object-cover">
                                <div class="p-2">
                                    <h3 class="font-medium">{{ $packet->title }}</h3>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="flex justify-center items-center mt-5 gap-8 text-3xl">
                <a href="{{ $otherPackets->previousPageUrl() }}" class="">
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="18" viewBox="0 0 11 18" fill="none">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0.390382 9.83505C0.140421 9.58501 0 9.24594 0 8.89238C0 8.53883 0.140421 8.19975 0.390382 7.94972L7.93305 0.407051C8.05604 0.279704 8.20317 0.178128 8.36584 0.108249C8.52851 0.0383705 8.70347 0.00158876 8.88051 5.03418e-05C9.05755 -0.00148808 9.23312 0.0322479 9.39699 0.0992891C9.56085 0.16633 9.70972 0.265334 9.83491 0.390525C9.9601 0.515715 10.0591 0.664584 10.1261 0.828446C10.1932 0.992307 10.2269 1.16788 10.2254 1.34492C10.2238 1.52196 10.1871 1.69692 10.1172 1.85959C10.0473 2.02226 9.94573 2.16939 9.81838 2.29238L3.21838 8.89238L9.81838 15.4924C10.0613 15.7439 10.1957 16.0807 10.1926 16.4303C10.1896 16.7798 10.0494 17.1143 9.80214 17.3615C9.55493 17.6087 9.22051 17.7489 8.87092 17.752C8.52132 17.755 8.18452 17.6206 7.93305 17.3777L0.390382 9.83505Z" fill="#373737"/>
                    </svg>
                </a>
                <span class="font-bold text-base mt-0.75">{{ $otherPackets->currentPage() }} of
                    {{ $otherPackets->lastPage() }}</span>
                <a href="{{ $otherPackets->nextPageUrl() }}"
                    class="rotate-180">
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="18" viewBox="0 0 11 18" fill="none" class="rotate-180">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M9.83496 7.9169C10.0849 8.16694 10.2253 8.50602 10.2253 8.85957C10.2253 9.21312 10.0849 9.5522 9.83496 9.80224L2.29229 17.3449C2.1693 17.4722 2.02217 17.5738 1.8595 17.6437C1.69683 17.7136 1.52187 17.7504 1.34483 17.7519C1.16779 17.7534 0.992217 17.7197 0.828356 17.6527C0.664494 17.5856 0.515624 17.4866 0.390433 17.3614C0.265244 17.2362 0.166239 17.0874 0.0991983 16.9235C0.0321569 16.7596 -0.00157833 16.5841 -4.00543e-05 16.407C0.00149822 16.23 0.0382805 16.055 0.108159 15.8924C0.178038 15.7297 0.279613 15.5826 0.40696 15.4596L7.00696 8.85957L0.40696 2.25957C0.164083 2.0081 0.0296907 1.6713 0.0327282 1.3217C0.0357656 0.972105 0.175991 0.637688 0.423202 0.390476C0.670413 0.143267 1.00483 0.00304031 1.35443 1.90735e-06C1.70402 -0.0030365 2.04082 0.131357 2.29229 0.374233L9.83496 7.9169Z" fill="#373737"/>
                    </svg>
                </a>
            </div>
        </div>

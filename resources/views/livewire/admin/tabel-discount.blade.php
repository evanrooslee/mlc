<div>
  ...
  @if ($flash_message)
    <div class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative"
      ...
      {{ $flash_message }}
      <button wire:click="clearFlashMessage" class="absolute top-0 right-0 mt-2 mr-2 text-green-700 hover:text-green-900">
      ...
      <thead>
        <tr class="bg-white">
          <th class="py-3 px-4 text-left">
            <button wire:click="sortByColumn('code')"
          ...
          <th class="py-3 px-4 text-left text-xs text-black">
            <button wire:click="sortByColumn('percentage')"
              class="flex items-center text-xs text-black hover:text-blue-600 transition-colors">
          ...
          <th class="py-3 px-4 text-left text-xs text-black">
            <button wire:click="sortByColumn('is_valid')"
              class="flex items-center text-xs text-black hover:text-blue-600 transition-colors">
              <span>Status</span>
              @if ($sortBy === 'is_valid')
                @if ($sortDirection === 'asc')
      ...
      <tbody>
        @forelse($discounts as $discount)
          <tr class="border-t border-[rgba(223,223,223,0.25)]">
            <td class="py-2 px-4 text-xs font-mono">{{ $discount->code }}</td>
            <td class="py-2 px-4 text-xs">{{ $discount->percentage }}%</td>
            <td class="py-2 px-4 text-xs">
              @if($discount->is_valid)
                <span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">Aktif</span>
              @else
                <span class="bg-red-100 text-red-800 px-2 py-1 rounded text-xs">Tidak Aktif</span>
              @endif
            </td>
            <td class="py-2 px-4 text-xs">{{ $discount->created_at->format('d/m/Y H:i') }}</td>
            <td class="py-2 px-4 text-xs">
              <div class="flex items-center gap-2">
                {{-- Edit Button --}}
                <button
                  wire:click="$dispatch('openModal', {component: 'admin.components.edit-discount-modal', arguments: {discountId: {{ $discount->id }}}})"
                  ...
                  <span class="text-xs text-[#68A2DD] group-hover:text-white transition-colors">Edit</span>
                </button>

                {{-- Delete Button --}}
                <button
                  wire:click="$dispatch('openModal', {component: 'admin.components.delete-discount-modal', arguments: {discountId: {{ $discount->id }}, code: '{{ $discount->code }}', percentage: {{ $discount->percentage }}}})"
                  ...
                  <span class="text-xs text-[#BA2B15] group-hover:text-white transition-colors">Hapus</span>
                </button>
              </div>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="5" class="py-8 px-4 text-center text-gray-500">
              Tidak ada data diskon yang ditemukan
            </td>
          </tr>
        @endforelse
      </tbody>
  ...
  <div class="mt-6">
    {{ $discounts->links('livewire.admin.pagination') }}
  </div>
</div>
